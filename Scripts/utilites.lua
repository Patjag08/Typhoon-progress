...
local parameters = 
{
    ...
    --MFD parameters
    MFDTEST1              =get_param_handle("MFDTEST"),
}
...
local HEA_state     = 0
--MFD
local MFDtest_switch = device_commands.MFDL17
local MFDTest_state = 0
--HUD
...
utilites:listen_command(MFDTest_state)
utilites:listen_command(MFDtest_switch)
------------------------------------------------------------------FUNCTION-POST-INIT---------------------------------------------------------------------------------------------------
...
function SetCommand(command,value)
    ...
    if command == MFDtest_switch and MFDTest_state == 0 then
        MFDTest_state = 1
    elseif command == MFDtest_switch and MFDTest_state == 1 then
        MFDTest_state = 0
    end
...
function update()
    ...
    parameters.MFDTEST1:set(MFDTest_state)
