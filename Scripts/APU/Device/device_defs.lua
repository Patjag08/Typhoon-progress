...
APU_IND_TEX_PATH                = LockOn_Options.script_path.."../Textures/HUD/"
...
APU_ASPECT_HEIGHT =  GetAspect()

APU_DEFAULT_LEVEL = 4
APU_DEFAULT_NOCLIP_LEVEL = APU_DEFAULT_LEVEL - 1
...
local APUScreen             = {255,255,255,200}
local APU_ladder_color      = {210,150,230,255}

APU_LINE           = MakeMaterial(APU_IND_TEX_PATH.."gear.dds", GreenColor)
...
function APU_vert_gen(width, height)
    return {{(0 - width) / 2 / default_APU_x , (0 + height) / 2 / default_APU_y},
    {(0 + width) / 2 / default_APU_x , (0 + height) / 2 / default_APU_y},
    {(0 + width) / 2 / default_APU_x , (0 - height) / 2 / default_APU_y},
    {(0 - width) / 2 / default_APU_x , (0 - height) / 2 / default_APU_y},}
end

function APU_duo_vert_gen(width, total_height, not_include_height)
    return {
        {(0 - width) / 2 / default_APU_x , (0 + total_height) / 2 / default_APU_y},
        {(0 + width) / 2 / default_APU_x , (0 + total_height) / 2 / default_APU_y},
        {(0 + width) / 2 / default_APU_x , (0 + not_include_height) / 2 / default_APU_y},
        {(0 - width) / 2 / default_APU_x , (0 + not_include_height) / 2 / default_APU_y},
        {(0 + width) / 2 / default_APU_x , (0 - not_include_height) / 2 / default_APU_y},
        {(0 - width) / 2 / default_APU_x , (0 - not_include_height) / 2 / default_APU_y},
        {(0 + width) / 2 / default_APU_x , (0 - total_height) / 2 / default_APU_y},
...
function calculateCircle(object, rAPUus, init_x, init_y, point_num)
	local verts = {}
    multiplier = math.rad(360.0/point_num)
    verts[1] = {init_x / default_APU_x, init_y / default_APU_y}
	for i = 2, point_num do
	  verts[i] = {(init_x + rAPUus * math.cos(i * multiplier)) / default_APU_x, (init_y + rAPUus * math.sin(i * multiplier)) / default_APU_y}
    end
    local indices = {}
