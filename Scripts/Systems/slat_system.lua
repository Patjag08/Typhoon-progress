...
function getIASKnots()
    ...
    G_LOAD = (sensor_data.getVerticalAcceleration())
...
function update()
    ...
    if (G_LOAD >= 6) then
        WING_TARGET = 1
    elseif (G_LOAD < 6) then
        WING_TARGET= G_LOAD/6
    end
    -- print_message_to_user(AOA)
    
    set_aircraft_draw_argument_value(19,SLATS_TARGET * 0.4)
    set_aircraft_draw_argument_value(20,SLATS_TARGET)
    set_aircraft_draw_argument_value(1053,SLATS_TARGET * 0.8)
    set_aircraft_draw_argument_value(1054,SLATS_TARGET * 0.8)
    -- set_aircraft_draw_argument_value(600,WING_TARGET)
end
