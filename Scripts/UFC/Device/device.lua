...
local ufc_fuel_f  = get_param_handle("UFC_FUEL_A")
local ufc_fuel_a  = get_param_handle("UFC_FUEL_F")
...
function update()
    ...
    ufc_fuel:set(math.floor(((sensor_data.getTotalFuelWeight())/100)+0.5)*100)
    ufc_fuel_f:set(math.floor(((sensor_data.getTotalFuelWeight())/100)+0.5)*60)
    ufc_fuel_a:set(math.floor(((sensor_data.getTotalFuelWeight())/100)+0.5)*40)
end